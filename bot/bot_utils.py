...
# Chỉ 6 loại phòng được chấp nhận. Mọi trường hợp khác → "Trọ thường"
ALLOWED_ROOM_TYPES = ["Studio", "Gác xép", "Giường tầng", "1N1K", "2N1K", "3N1K"]
ROOM_TYPE_DEFAULT = "Trọ thường"
...
def normalize_room_type(text):
    ...
    Nếu không nhận dạng được → trả về 'Trọ thường'.
    Hỗ trợ cả nhãn đã đúng (Studio, 1N1K...) và văn bản thô."""
    if not text:
        return ROOM_TYPE_DEFAULT

    raw = str(text).strip()

    # Nếu đã là nhãn đúng rồi thì trả về luôn
    if raw in ALLOWED_ROOM_TYPES:
        return raw

    # Normalize: bỏ dấu, lowercase, giữ a-z0-9 và dấu cách
...
def resolve_symbol_for_group_name(group_name, group_symbols):
    ...
    Lưu ý: tier "g in d" với tên nhóm quá ngắn (vd chỉ "1") sẽ khớp nhầm vào "11a", "2n1k"...
    nên chỉ dùng tier đó khi len(g) >= MIN_SUBSTRING_GROUP_LEN.
    """
    ...
    for dv_name, symbol in group_symbols.items():
        ...
        if d == g:
            ...
            if len(g) < min_substring_group_len:
                continue
            tier, score = 1, len(g)
        else:
            continue
...
def _generate_rules():
    ...
    custom_rules = {
        ...
        "tc 2": {"add_prefix": True, "remove_phone": True},
        "tc2": {"add_prefix": True, "remove_phone": True},
        "tc 3": {"add_prefix": True, "remove_phone": True},
        "tc3": {"add_prefix": True, "remove_phone": True},
        "tc 4": {"add_prefix": True, "remove_phone": True},
        "tc4": {"add_prefix": True, "remove_phone": True},
        ...
        "tuananh chdv 1": {"add_prefix": True, "format_price": True, "remove_phone": True},
        ...
        "tuananh chdv 2": {"add_prefix": True, "format_price": True, "remove_phone": True},
        ...
        "invest": {"add_prefix": True},
        "family": {"add_prefix": True, "remove_commission": True},
        "hm": {"add_prefix": True},
...
class ProcessLock:
    def __init__(self, name, bot_dir):
        self.name = name
        self.lock_file = os.path.join(bot_dir, f"{name}.lock")
        self.fd = None
