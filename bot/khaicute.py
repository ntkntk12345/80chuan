...
# Chỉ 6 loại được phép. Mọi trường hợp khác → "Trọ thường"
_ALLOWED_ROOM_TYPES = ["Studio", "Gác xép", "Giường tầng", "1N1K", "2N1K", "3N1K"]
_ROOM_TYPE_DEFAULT  = "Trọ thường"
_ROOM_TYPE_PATTERNS = [
    ("Studio",        ["studio"]),
    ("Gác xép",       ["gac xep", "duplex", "loft", "mezzanine"]),
    ("Giường tầng",   ["giuong tang", "ktx", "bedspace", "dorm"]),
    ("1N1K",          ["1n1k", "1pn1k", "1 ngu 1 khach", "1 phong ngu 1 khach", "1ngu1khach", "1 bedroom"]),
    ("2N1K",          ["2n1k", "2pn1k", "2 ngu 1 khach", "2 phong ngu 1 khach", "2ngu1khach", "2 bedroom"]),
    ("3N1K",          ["3n1k", "3pn1k", "3 ngu 1 khach", "3 phong ngu 1 khach", "3ngu1khach", "3 bedroom"]),
]


def normalize_room_type(text):
    """Chuẩn hóa loại phòng về 6 dạng cố định.
    Nếu không nhận dạng được → 'Trọ thường'.
    """
    if not text:
        return _ROOM_TYPE_DEFAULT
    raw = str(text).strip()
    if raw in _ALLOWED_ROOM_TYPES:
        return raw
    # Bỏ dấu, lowercase, chỉ giữ a-z0-9 + dấu cách
    norm = unicodedata.normalize("NFD", raw)
    norm = "".join(ch for ch in norm if unicodedata.category(ch) != "Mn")
    norm = re.sub(r"[^a-z0-9]+", " ", norm.lower()).strip()
    for label, patterns in _ROOM_TYPE_PATTERNS:
...
def _build_standard_room_summary(property_item, district_key, full_info=None):
    ...
    room_type = normalize_room_type(property_item.get("roomType") or "")
...
def send_room_to_zalo(room_id, district_key):
    ...
    if photos or videos:
        threading.Thread(
            target=_send_media,
            args=(photos[:10], videos[:3], target_id, room_id, True), # send_sticker=True
            daemon=True
        ).start()
...
def _send_media(photos, videos, dest_id, room_id=None, send_sticker=False):
    """Gửi ảnh + video (chạy nền, giống khaicute)"""
    bot = get_bot()
    if not bot:
        return
    
    # URL xem ảnh trên web (giả định chạy ở cổng 8000 như trong create_task)
    # Nếu có public IP thì nên thay thế localhost bằng IP đó.
    room_url = f"http://163.227.230.41:8000/anh/{room_id}" if room_id else ""
    
    image_paths = []
    if photos:
        valid = [p for p in photos if p.get("url")]
        def dl(idx, photo):
            url = photo["url"]
            try:
                r = requests.get(url, stream=True, timeout=15)
                r.raise_for_status()
                tmp = f"tmp_web_{int(time.time()*1000)}_{idx}.jpg"
                with open(tmp, "wb") as f:
                    for chunk in r.iter_content(8192):
                        if chunk: f.write(chunk)
                return {"idx": idx, "path": tmp, "width": photo.get("width", 2560), "height": photo.get("height", 1920)}
            except Exception as e:
                print(f"[MEDIA] Lỗi tải ảnh {idx}: {e}")
                return None
        
        futures = [_executor.submit(dl, i, p) for i, p in enumerate(valid)]
        ...
        if image_paths:
            paths = [x["path"] for x in image_paths]
            try:
                bot.sendMultiLocalImage(
                    paths, dest_id, ThreadType.USER,
                    width=image_paths[0]["width"],
                    height=image_paths[0]["height"]
                )
                print(f"[MEDIA] Đã gửi {len(paths)} ảnh")
            except Exception as e:
                print(f"[MEDIA] Lỗi sendMultiLocalImage: {e}")
                
                # Check nếu lỗi do bị limit (Zalo thường trả về lỗi liên quan đến limit/402/429/221)
                err_str = str(e).lower()
                is_limited = (
                    "limit" in err_str or 
                    "402" in err_str or 
                    "429" in err_str or 
                    "221" in err_str or 
                    "vượt quá" in err_str or
                    "thumburl" in err_str  # KeyError do lỗi 221 trả về cấu trúc lạ
                )
                
                if is_limited and room_id:
                    print(f"[MEDIA] Phát hiện account bị limit gửi ảnh. Gửi link thay thế cho {dest_id}")
                    limit_msg = (
                        f"Vì zalo giới hạn ảnh nên mình xem qua website giúp bên em nhé: {room_url}"
                    )
                    try: bot.send(Message(text=limit_msg), dest_id, ThreadType.USER)
                    except: pass
                    # Không thử gửi lẻ nữa nếu đã biết bị limit
                else:
                    # Nếu không phải limit rõ ràng, thử gửi từng tấm
                    for item in image_paths:
                        try:
                            bot.sendLocalImage(item["path"], dest_id, ThreadType.USER)
                            time.sleep(1)
                        except Exception as e2:
                            err2 = str(e2).lower()
                            is_limit2 = any(x in err2 for x in ["limit", "402", "429", "221", "vượt quá"])
                            if is_limit2 and room_id:
                                limit_msg = f"Vì zalo giới hạn ảnh nên mình xem qua website giúp bên em nhé: {room_url}"
                                try: bot.send(Message(text=limit_msg), dest_id, ThreadType.USER)
                                except: pass
                                break
                            pass
        ...
        for video in videos:
            try:
                bot.sendRemoteVideo(
                    ...
                    dest_id, ThreadType.USER,
                    width=video.get("width", 1280),
                    height=video.get("height", 720)
                )
                time.sleep(0.5)
            except Exception as e:
                print(f"[MEDIA] Lỗi gửi video: {e}")

    # Gửi sticker cuối cùng nếu yêu cầu
    if send_sticker:
        try:
            # Sticker ID 2, Package ID 1 là sticker mặt cười/like phổ biến
            bot.send(Message(sticker_id="2", package_id="1"), dest_id, ThreadType.USER)
        except:
            pass
...
def api_create_task():
    ...
    try:
        ...
        for i, room in enumerate(top, 1):
            ...
            videos = room_full.get("videos", []) if room_full else []
            if photos or videos:
                # Để đảm bảo không loạn thứ tự giữa các căn, ta gửi media ngay (không thread nếu muốn siêu chuẩn, 
                # nhưng Thread ở đây giúp web ko treo lâu. Zalo sẽ tự xếp queue tin nhắn của 1 account).
                _send_media(photos[:10], videos[:2], uid, room_id, send_sticker=True)
                time.sleep(1.5) # Chờ một chút trước khi sang căn tiếp theo
