...
# ── Singleton guard (chẹn TRƯỚC LoggerTee để không log 2 lần) ─────────────────
...
# Duplicates stdout/stderr to log file so web panel still displays logs
...
log_file_path = os.path.join(BOT_DIR, f"{SERVICE_NAME}.log")
if not hasattr(sys.stdout, 'log'):
    sys.stdout = LoggerTee(log_file_path, sys.stdout)
if not hasattr(sys.stderr, 'log'):
...
class ZaloListener(ZaloAPI):
    def __init__(self, api_key, secret_key, listener_acc):
        ...
        # Quận/huyện (daura.json) — dùng tách session khi gửi hàng loạt tin khác khu (vd Thanh Xuân → Hà Đông)
        self._aura_kw, self._kw_levels, _kw_parents = bot_utils.load_daura_keywords()
        self._district_keywords_for_session = sorted(
            [
                k
                for k in self._aura_kw
                if self._kw_levels.get(k) in ("district", "area")
            ],
            key=lambda x: len(unicodedata.normalize("NFC", str(x))),
            reverse=True,
        )
    ...
    def _norm_district_token(self, name):
        if not name:
            return ""
        return unicodedata.normalize("NFC", str(name)).strip().lower()

    def _first_district_in_text(self, text):
        """Quận/huyện (hoặc area) xuất hiện sớm nhất trong text; ưu tiên chuỗi khớp dài hơn khi cùng vị trí."""
        if not text or not self._district_keywords_for_session:
            return None
        text_norm = unicodedata.normalize("NFC", text).lower()
        best = None  # (start_pos, -len(kw), keyword) — tuple sort: leftmost, then longest
        for keyword in self._district_keywords_for_session:
            kw_norm = unicodedata.normalize("NFC", str(keyword)).lower()
            pattern = r"(?<!\w)" + re.escape(kw_norm) + r"(?!\w)"
            m = re.search(pattern, text_norm)
            if not m:
                continue
            cand = (m.start(), -len(keyword), keyword)
        ...
        with self.session_lock:
            ...
            if content_type == "text":
                ...
                if isinstance(content_data, dict):
                    txt_str = content_data.get("original_text") or content_data.get("text") or ""
                else:
                    txt_str = str(content_data or "")

                is_new_listing = False
                if txt_str:
                    txt_clean = txt_str.strip()
                    lower_txt = txt_clean.lower()
                    starts_with_emoji = any(txt_clean.startswith(emoji) for emoji in ["🌹", "🏡", "📍", "🚪", "🛑", "💸", "🔥", "📌", "🔑"])
                    has_listing_keywords = any(kw in lower_txt for kw in ["địa chỉ", "đ/c", "đc", "dc:", "d/c:", "giá", "mã", "phí dịch vụ", "vskk", "nội thất", "thang máy", "cọc"])
                    is_long_text = len(txt_clean) > 150
                    if starts_with_emoji or has_listing_keywords or is_long_text:
                        is_new_listing = True

                if is_new_listing and buffer_check:
                    has_media = len(buffer_check.get("photos", [])) > 0 or len(buffer_check.get("videos", [])) > 0
                    if has_media:
                        # Text dài/listing mới + buffer đã có media = BÀI ĐĂNG MỚI → đóng session cũ
                        print(f"[SESSION] Listing mới + media cũ → đóng session cũ của {symbol}")
                        self._flush_immediate(session_key)
                        buffer_check = {}
                    elif len(buffer_check.get("texts", [])) > 0:
                        # Text dài/listing mới + buffer đã có text khác (chưa có media) = text mới thay thế
                        # Chỉ đóng nếu là nhóm thường
                        if not is_special:
                            print(f"[SESSION] Listing mới + text cũ → đóng session cũ của {symbol}")
                            self._flush_immediate(session_key)
                            buffer_check = {}

                # Đổi quận/huyện rõ ràng → coi là bài đăng khác (tránh gộp Thanh Xuân + Hà Đông + Cầu Giấy một session)
                if buffer_check and self._district_keywords_for_session:
                    if isinstance(content_data, dict):
                        scan_txt = str(
                            content_data.get("original_text") or content_data.get("text") or ""
                        ).strip()
                    else:
                        scan_txt = str(content_data or "").strip()
                    new_district = self._first_district_in_text(scan_txt)
                    anchor = buffer_check.get("session_anchor_district")
                    has_stuff = (
                        len(buffer_check.get("texts", [])) > 0
                        or len(buffer_check.get("photos", [])) > 0
                        or len(buffer_check.get("videos", [])) > 0
                    )
                    if (
                        new_district
                        and anchor
                        and has_stuff
                        and self._norm_district_token(new_district)
                        != self._norm_district_token(anchor)
                    ):
                        print(
                            f"[SESSION] Đổi khu vực ({anchor} → {new_district}), tách session — {symbol}"
                        )
                        self._flush_immediate(session_key)
                        buffer_check = {}
            ...
            if content_type == "text":
                ...
                scan_for_anchor = (text_obj.get("original_text") or text_obj.get("text") or "").strip()
                d0 = self._first_district_in_text(scan_for_anchor)
                if d0 and buffer.get("session_anchor_district") is None:
                    buffer["session_anchor_district"] = d0
                    ...
                    if is_special:
                        ...
                        with self.session_lock:
                            ...
                            if text:
                                ...
                                starts_with_emoji = any(text_clean.startswith(emoji) for emoji in ["🌹", "🏡", "📍", "🚪", "🛑", "💸", "🔥", "📌", "🔑"])
                                has_listing_keywords = any(kw in lower_txt for kw in ["địa chỉ", "đ/c", "đc", "dc:", "d/c:", "giá", "mã", "phí dịch vụ", "vskk", "nội thất", "thang máy", "cọc"])
                                is_long_text = len(text_clean) > 150
                                if starts_with_emoji or has_listing_keywords or is_long_text:
                                    is_new_listing = True
                            is_first = not buffer or not buffer.get("texts") or is_new_listing
...
if __name__ == "__main__":
    ...
    # Process lock is already handled at the top of the file via _singleton_fd
        
    # Write PID file for Node.js status checks
    try:
        with open(os.path.join(BOT_DIR, f"{SERVICE_NAME}.pid"), "w", encoding="utf-8") as f:
